...
export class NvSearchComponent implements OnInit {
  ...
  constructor(
    ...
    private advanceFilter: NvAdvanceFilterPipe
  ) {}
  ...
  ngOnInit(): void {
    ...
    this.queryFormControl.valueChanges.subscribe((selectedValue) => {
      ...
      if (this.isAdvance) {
        ...
        this.filteredList = this.advanceFilter
          ?.transform(
            this._list,
        ...
        this.filteredList = this.filter
          ?.transform(this._list, this.queryFormControl.value)
          ?.map((item: any) => item);
      }
      if (this.filteredList) {
        ...
        this.filteredItems
          .asObservable()
          .subscribe((list) => console.log('outPut' + JSON.stringify(list)));
      } else {
